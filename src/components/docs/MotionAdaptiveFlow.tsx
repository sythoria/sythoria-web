...
import { Sliders, Settings, Cpu, Activity } from "lucide-react";
...
const STEPS: Step[] = [
  {
    ...
    description: "Launch motion controllers and query environment state.",
    logicCheck: "Check if animations are enabled",
  },
  {
    id: 2,
    title: "2. Accessibility Check",
    icon: Settings,
    iconBg: "bg-orange-500/10 dark:bg-orange-500/15",
    iconColor: "text-orange-600 dark:text-orange-400 border-orange-500/20",
    description: "Inspect OS preferences and manual accessibility options.",
    logicCheck: "prefers-reduced-motion check",
  },
  {
    id: 3,
    title: "3. CPU Core Check",
    icon: Cpu,
    iconBg: "bg-amber-500/10 dark:bg-amber-500/15",
    iconColor: "text-amber-600 dark:text-amber-400 border-amber-500/20",
    description: "Query system hardware concurrency and core availability.",
    logicCheck: "Cores <= 4 ➔ Downgrade Motion",
  },
  {
    id: 4,
    title: "4. Runtime FPS Guard",
    icon: Activity,
    iconBg: "bg-purple-500/10 dark:bg-purple-500/15",
    iconColor: "text-purple-600 dark:text-purple-400 border-purple-500/20",
    description: "Monitor execution speed dynamically and disable filters.",
    logicCheck: "FPS < 45 ➔ Safe Mode fallback",
  },
];

export default function MotionAdaptiveFlow() {
  return (
    <div className="my-8 w-full rounded-2xl border border-border/40 bg-surface/20 dark:bg-black/15 backdrop-blur-md p-5 md:p-6 flex flex-col gap-6 select-none shadow-lg overflow-hidden">
      <style>{`
        ...
        .flow-line-motion {
          ...
          animation: motionFlow 1.6s infinite linear;
